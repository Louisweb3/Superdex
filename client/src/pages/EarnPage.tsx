...
import { ArrowUpRight, Zap, Globe, CheckCircle2, Circle, ExternalLink, Coins, Twitter, MessageCircle, Heart, Users, Share2, Star, Flame, Anchor } from "lucide-react";
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-5 py-20">
      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-[14px] text-[#8b97aa]">Connect to view and complete earn tasks</p>
      </div>
      <button
        onClick={onConnect}
...
function TaskCard({ task, onClaim }: { task: any; onClaim: () => void }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[22px] border border-[#182332] bg-[#060d17] p-4 sm:p-5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-[2px]">
      <div className="absolute right-[-20px] top-[-20px] h-[90px] w-[90px] rounded-full blur-[60px]" style={{ background: `${glow}22` }} />
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            <div
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] border text-[20px]"
              style={{ background: `${glow}15`, borderColor: `${glow}55` }}
            >
              <Icon className="h-5 w-5" style={{ color: glow }} />
            </div>
            <div className="min-w-0">
              <h3 className="text-[15px] font-bold text-white">{task.title}</h3>
              <p className="mt-1 text-[13px] text-[#8c98aa] leading-relaxed">{task.description}</p>
            </div>
          </div>
...
export function EarnPage(): JSX.Element {
  ...
  const [activeCategory, setActiveCategory] = useState<"onchain" | "offchain">("onchain");
  ...
  const totalXP = filtered.reduce((sum, t) => sum + (t.completed ? t.xp_reward : 0), 0);
      ...
      <div className="w-full px-[14px] pb-[20px] pt-[12px] sm:px-[18px]">
        ...
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[13px] text-[#6f7b8e]">Complete tasks to earn XP & cashback</p>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <div className="relative overflow-hidden rounded-[22px] border border-[#1b2432] bg-gradient-to-br from-[#071321] via-[#08111d] to-[#02050b] p-4 sm:p-5 backdrop-blur-xl">
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-[#6f7b8e]">{activeCategory} Tasks</p>
                  <p className="mt-1 text-[26px] font-black text-white">
                    {completedCount} <span className="text-[16px] font-bold text-[#7f8b9d]">/ {filtered.length}</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] uppercase tracking-[0.22em] text-[#6f7b8e]">XP Earned</p>
                  <p className="mt-1 text-[26px] font-black text-[#3acd5b]">+{totalXP}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 rounded-[16px] border border-[#182332] bg-[#060d17] p-1.5">
              {(["onchain", "offchain"] as const).map((cat) => {
                ...
                const glow = cat === "onchain" ? onchainGlow : offchainGlow;
                return (
                  <button
                    key={cat}
                    ...
                    data-testid={`tab-earn-${cat}`}
                  >
                    <CatIcon className="h-4 w-4" style={{ color: isActive ? glow : "#6f7b8e" }} />
                    {cat === "onchain" ? "Onchain" : "Offchain"}
                  </button>
                );
              })}
            </div>
            ...
            <div className="grid gap-3">
              ...
              {filtered.length === 0 && (
                <div className="flex flex-col items-center justify-center gap-3 py-14 rounded-[22px] border border-[#182332] bg-[#060d17]">
                  <Anchor className="h-10 w-10 text-[#4a5568]" />
                  <p className="text-[14px] text-[#6f7b8e]">No tasks available in this category</p>
                </div>
              )}
            </div>
          </div>
