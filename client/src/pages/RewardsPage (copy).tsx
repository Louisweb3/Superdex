...
function StatCard({
  ...
  return (
    <article className="flex flex-col gap-1.5 rounded-[14px] sm:rounded-[18px] border border-[#131b27] bg-[#00040e] px-4 py-4 sm:px-5 sm:py-5">
      ...
      {sub && (
        <p className="font-['Inter',sans-serif] text-[11px] sm:text-[12px] text-[#667082]">
          {sub}
        </p>
      )}
    </article>
...
function QuestRow({ quest, wallet }: { quest: DailyQuest; wallet: string }) {
  ...
  const labels: Record<string, { name: string; desc: string; icon: string }> = {
    swaps: {
      ...
      desc: "Make any 3 swaps today",
      icon: "⚡",
    },
    volume: {
      name: "Trade $100 Volume",
      desc: "Trade at least $100 in total",
      icon: "📈",
    },
    login: {
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4 rounded-[12px] sm:rounded-[14px] border border-[#131b27] bg-[#00040e] px-4 py-3 sm:px-5 sm:py-4">
        ...
        <div className="mt-1.5 flex items-center gap-2">
          <div className="h-[4px] flex-1 overflow-hidden rounded-full bg-[#101723]">
            <div
              ...
              style={{
                width: `${pct}%`,
                background: quest.completed ? "#2dae50" : "#1a5cb0",
              }}
            />
          </div>
      ...
      {quest.claimed && (
        <span className="shrink-0 font-['Inter',sans-serif] text-[12px] text-[#2dae50]">
          ✓
        </span>
      )}
    </div>
...
function LeaderRow({
  ...
  return (
    <div
      className={`flex items-center gap-3 border-b border-[#0d1624] py-3 last:border-0 ${isMe ? "rounded-[8px] bg-[#081420] px-2" : ""}`}
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="font-['Inter',sans-serif] text-[13px] sm:text-[14px] font-medium text-[#c8ccd4] truncate">
          {isMe
            ? "You (" + shortWallet(user.wallet_address) + ")"
...
function QuestTimer() {
  ...
  useEffect(() => {
    const tick = () =>
      setSecs(Math.max(0, Math.floor((nextMidnightUTC() - Date.now()) / 1000)));
    tick();
    ...
    <div className="flex flex-col items-center justify-center gap-5 py-16 sm:py-20">
      <div className="flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-full bg-[#071020] border border-[#131b27]">
        <img
          ...
          alt="Wallet"
          className="h-8 w-8 sm:h-10 sm:w-10 object-contain opacity-60"
        />
      </div>
        ...
        <div className="mb-4 sm:mb-5 flex items-center justify-between">
          ...
          {wallet.isConnected && addr && (
            <div className="flex flex-col items-end gap-0.5">
              ...
              <span className="font-['Inter',sans-serif] text-[10px] text-[#6c778a]">
                Base Mainnet
              </span>
            </div>
          )}
          ...
          <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <StatCard
                ...
                value={fmtUsd(user.cashback_usd)}
                sub="Lifetime cashback"
                icon="💰"
              />
              <StatCard
                label="Your Streak"
                value={`${user.streak} DAYS`}
                sub={user.streak > 0 ? "Keep it going!" : "Start today!"}
                icon="🔥"
              />
              <StatCard
                label="Total Swaps"
                value={user.total_swaps.toString()}
                sub={`${fmtUsd(user.total_volume_usd)} volume`}
                icon="⚡"
              />
              <StatCard
                label="Weekly XP"
            ...
            <section>
              ...
              <div className="flex flex-col gap-2.5 sm:gap-3">
                {quests
                  ? quests.map((q) => (
                      <QuestRow key={q.id} quest={q} wallet={addr!} />
                    ))
                  : [0, 1, 2].map((i) => (
                      <div
                        key={i}
                        className="h-16 animate-pulse rounded-[12px] bg-[#0d1624]"
                      />
                    ))}
              </div>
            </section>
