...
export function RewardsView({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-[1420px]">
      {/* GM card */}
      ...
      <GlassCard className="p-6">
        ...
        {gnClaimed ? (
          <div className="flex flex-col items-center gap-3 py-4">
            <div className="flex items-center gap-2 text-[#7A8CFF] text-[14px] font-semibold">
              <CheckCircle size={19} />
              GN sent today!
            </div>
            <p className="text-[#8B97A8] text-[13px] text-center">
              Come back tomorrow to send another GN and keep your streak
              alive.
            </p>
            {gnTxHash && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-[12px] text-[#7A8CFF] hover:text-[#5AE4A8] transition-colors"
              >
                <ExternalLink size={12} />
                View transaction
              </a>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-br from-[#7A8CFF] to-[#5A6EE0] hover:brightness-110 disabled:opacity-40 text-white font-semibold text-[15px] h-[50px] rounded-[14px] shadow-[0_4px_24px_rgba(122,140,255,0.25)] transition-all duration-200"
          >
            {sendingGn ? (
              <Loader2 size={18} className="animate-spin" />
            ) : (
        ...
        <div className="mt-5 border-t border-white/[0.06] pt-4">
          <div className="text-[11px] text-[#5E6B7A] text-center">
            GN is stored on-chain as{" "}
            <code className="text-[#8B97A8]">0x474e</code> (UTF-8 encoded).
            Each GN uses a tiny amount of ETH for gas.
          </div>
        </div>
      </GlassCard>
